...
export class HistorialProvider {
  ...
  private crear_mail( texto:string ){

    if( !this.platform.is('cordova') ){
      console.warn('No puedo crear un email, estoy en la pc.');
      return;
    }

    this.emailComposer.isAvailable().then((available: boolean) =>{
          
          let mensaje:any = texto.split(";");
          let email = {
            to: mensaje[0].replace("MATMSG:TO:",""),
            cc: '',
            bcc: [],
            attachments: [
            ],
            subject: mensaje[1].replace("SUB:",""),
            body: mensaje[2].replace("BODY:",""),
            isHtml: true
          }
  ...
  private crear_contacto( texto:string ){
    ...
    if( !this.platform.is('cordova') ){
      console.warn('No puedo crear un contacto, estoy en la pc.');
      return;
    }
